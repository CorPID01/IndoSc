...
Exp=$(curl -sS https://raw.githubusercontent.com/CorpID01/IP/main/regis | grep $MYIP | awk '{print $3}')
fi
...
Exp1=$(curl https://raw.githubusercontent.com/CorpID01/IP/main/regis | grep $MYIP | awk '{print $3}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
export sem=$( curl -s https://raw.githubusercontent.com/CorPID01/IndoSc/main/versi)
